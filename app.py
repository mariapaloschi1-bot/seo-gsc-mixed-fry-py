...
# ============================================
# CSS CUSTOM CON CONTRASTO MIGLIORATO
# ============================================
st.markdown("""
<style>
    ...
    * {
    ...
    /* FIX SELECTBOX - TESTO BIANCO SU SFONDO SCURO */
    ...
    /* FIX DROPDOWN MENU - CONTRASTO ALTO */
    ...
    /* FIX INPUT FIELDS - CONTRASTO MIGLIORATO */
    input[type="text"],
    input[type="number"],
    input[type="password"] {
        background-color: #2d3748 !important;
    ...
    /* FIX FILE UPLOADER - SFONDO SCURO CONTRASTATO */
    ...
    /* Holiday buttons - MIGLIORATI - multiriga, colori diversi */
    ...
    .business-event-card.drop:hover {
...
# ============================================
# CATEGORIZZAZIONE URL MIGLIORATA - CMS AWARE
# ============================================
def categorize_urls_advanced(pages_df):
    ...
    for _, row in pages_df.iterrows():
        ...
        # Date in URL (blog articles)
        elif any(f'/{year}/' in path for year in range(2015, 2026)):
            categories['Blog/News'].append(row)
            classified = True
        ...
        # MAGENTO CATEGORIES: .html without product ID patterns
        ...
        # Fallback: if contains numbers at end -> product, else -> category
        if not classified:
            # Check last 15 characters for digits
...
def filter_by_holiday(date_df, holiday_name):
    ...
    for year in range(min_year, max_year + 1):
        ...
        holiday_dates.append((start_date, end_date))
    
    mask = pd.Series([False] * len(date_df), index=date_df.index)
    for start, end in holiday_dates:
        mask |= (date_df['Date'] >= start) & (date_df['Date'] <= end)
    
    return date_df[mask]

# ============================================
# CALCOLO CTR TARGET MIGLIORATO
# ============================================
def calculate_target_ctr(position, current_ctr):
    """
    Calcola CTR target realistico basato su posizione e CTR attuale.
    Usa curve CTR realistiche per SERP Google.
    """
    # CTR baseline per posizione (dati da studi Backlinko/Sistrix)
    ctr_benchmarks = {
        1: 28.5,
        2: 15.7,
        3: 11.0,
    ...
    # Se il CTR attuale è già vicino o sopra il baseline, target conservativo
    if current_ctr >= baseline * 0.85:
        target = current_ctr * 1.15  # +15% miglioramento
    else:
        # Calcola un target intermedio tra CTR attuale e baseline
        gap = baseline - current_ctr
        target = current_ctr + (gap * 0.6)  # 60% del gap
    
    # Cap massimo realistico
    max_realistic_ctr = baseline * 1.3
    target = min(target, max_realistic_ctr)
...
# ============================================
with tab1:
    st.markdown("### 📊 Picchi e Cali nelle Performance")
    st.markdown("*Analisi automatica delle variazioni significative nei clic*")
        ...
        if events:
            ...
            for idx, event in enumerate(events[:5]):
                with cols[idx]:
                    ...
                    label = "PICCO PERFORMANCE" if event['type'] == 'peak' else "CALO PERFORMANCE"
                    color = COLOR_PALETTE['success'] if event['type'] == 'peak' else COLOR_PALETTE['danger']
                    card_class = "business-event-card" if event['type'] == 'peak' else "business-event-card drop"
                    
                    st.markdown(f"""
                    <div class="{card_class}">
                        <div style="font-size: 2.5rem; margin-bottom: 8px;">{icon}</div>
                        <div style="font-weight: 700; color: {color}; font-size: 0.95rem; margin-bottom: 8px;">{label}</div>
            ...
            st.info("📊 Nessuna variazione significativa rilevata nel periodo analizzato")
    ...
    # Pulsanti festività migliorati con colori diversi
    st.markdown('<div class="holiday-btn-container">', unsafe_allow_html=True)
    
    holiday_cols = st.columns(5)
    for idx, holiday in enumerate(HOLIDAYS):
        col_idx = idx % 5
        with holiday_cols[col_idx]:
            if st.button(
                f"{holiday['emoji']}\n{holiday['name']}", 
...
# ============================================
# TAB 2: GEOGRAPHIC - BUBBLE CHART MIGLIORATO
# ============================================
with tab2:
    ...
    if st.session_state.countries_data is not None:
        ...
        st.markdown("---")
        st.markdown("### 💎 Mercati con Opportunità - Analisi Visiva")
        ...
        if not opportunity_markets.empty:
            st.markdown(f"**🌍 {len(opportunity_markets)} mercati con alto potenziale identificati**")
            
            # BUBBLE CHART MIGLIORATO
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    ...
                    color=colors,
                    colorscale=[
                        [0, '#ef4444'],      # Red per CTR basso
                        [0.5, '#f59e0b'],    # Orange
                        [1, '#10b981']       # Green per CTR alto
                    ],
            ...
            fig.update_layout(
                ...
                xaxis=dict(
                    title="Impressioni",
                    gridcolor='#1f4068',
                    type='log'  # Scala logaritmica per meglio visualizzare range ampi
                ),
                yaxis=dict(
                    title="CTR %",
                    gridcolor='#1f4068'
            ...
            st.markdown("---")
            st.markdown("#### 📊 Dettaglio Top Opportunità")
            ...
            st.info("Nessun mercato opportunità trovato con i criteri attuali")
...
# ============================================
# TAB 4: BUSINESS - CTR TARGET MIGLIORATO
# ============================================
with tab4:
    ...
    if st.session_state.pages_data is not None:
        ...
        st.markdown("*CTR target calcolati in base a benchmark realistici per posizione SERP*")
        ...
        if st.session_state.queries_data is not None:
            ...
            if not opportunities.empty:
                for _, opp in opportunities.iterrows():
                    ...
                    # USA FUNZIONE MIGLIORATA
                    target_ctr = calculate_target_ctr(position, current_ctr)
                    ...
                    <div style="background: linear-gradient(135deg, rgba(0, 212, 255, 0.1), rgba(78, 205, 196, 0.1)); 
                        ...
                        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 15px;">
                            ...
                            <div style="text-align: center;">
                                ...
                                <div style="color: #e0e7ff; font-size: 0.9rem;">CTR Attuale</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 1.5rem; font-weight: 700; color: {COLOR_PALETTE['primary']};">{position:.1f}</div>
                        ...
                        <div style="background: rgba(78, 205, 196, 0.2); padding: 15px; border-radius: 10px; border-left: 4px solid {COLOR_PALETTE['secondary']};">
                            <div style="color: {COLOR_PALETTE['secondary']}; font-weight: 600; font-size: 1rem;">
                                🎯 CTR Target Realistico: {target_ctr:.2f}% (+{improvement:.0f}%)
                            </div>
                            <div style="color: white; font-size: 1.2rem; font-weight: 700; margin-top: 8px;">
                                Potenziale: +{potential_clicks if potential_clicks > 0 else 0:,} clic mensili
                            </div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
            else:
                st.info("Nessuna opportunità trovata con i filtri attuali")
