...
def parse_csv_robust(uploaded_file):
    """Parser CSV robusto - SUPPORTA ITALIANO E INGLESE"""
    try:
        ...
        for encoding in encodings:
            try:
                ...
                # MAPPING BILINGUE: Supporta CSV in Italiano E Inglese
                column_mapping = {
                    # ITALIANO (Google Search Console IT)
                    'Data': 'Date',
                    'Clic': 'Clicks',
                    ...
                    # INGLESE (Google Search Console EN)
                    # Date, Clicks, Impressions, CTR, Position, Country, Device → già in formato corretto
                    'Top queries': 'Query',  # Colonna query in GSC inglese
                    'Top pages': 'Page'  # Colonna pagine in GSC inglese
                }
...
def categorize_urls_advanced(pages_df):
    ...
    for _, row in pages_df.iterrows():
        ...
        # Blog/News patterns - MIGLIORATO con più pattern
        elif any(x in path for x in ['/blog/', '/news/', '/magazine/', '/articoli/', '/post/', 
                                      '/notizie/', '/article/', '/stories/', '/insights/']):
            categories['Blog/News'].append(row)
            classified = True
        
        # Date in URL (blog articles) - format: /2021/12/ or /2021-12- or /12/2021/
        elif any(f'/{year}/' in path or f'/{year}-' in path or f'-{year}/' in path 
        ...
        elif any(x in path for x in ['/item/', '/items/', '/articolo/', '/sku-', '/prod-', '/id-']):
            categories['Prodotti'].append(row)
            classified = True
        
        # Generic category patterns - SOLO se non ha molte parole nel path
        elif any(x in path for x in ['/categor', '/cat/']):
            # Se ha /c/ ma anche molti trattini (path lungo), probabilmente è prodotto
            if '/c/' in path:
                path_segments = [s for s in path.split('/') if s]
                if path_segments and '-' in path_segments[-1]:
                    word_count = len(path_segments[-1].split('-'))
                    if word_count >= 4:  # Path con 4+ parole tipo "abiti-lunghi-eleganti-rossi"
                        categories['Prodotti'].append(row)
                        classified = True
                    else:
                        categories['Categorie'].append(row)
        ...
        # MIGLIORAMENTO FALLBACK: Analisi intelligente URL
        if not classified:
            ...
            if path_segments:
                ...
                # Rimuovi estensioni per analisi pulita
                last_segment_clean = last_segment.replace('.html', '').replace('.htm', '').replace('.php', '')
                
                # PRODOTTO se:
                # 1. Ha numeri alla fine (SKU) tipo: "abiti-123" o "rossetto-45rt"
                # 2. Ha path molto lungo con molte parole (descrittivo) tipo: "abiti-lunghi-eleganti-rossi-cerimonia"
                # 3. Ha alta densità di numeri negli ultimi 10 caratteri
                ...
                # Conta parole nel path (separati da - o _)
                words_in_segment = len([w for w in last_segment_clean.replace('_', '-').split('-') if w])
                
                # Densità numeri negli ultimi 15 caratteri
                digit_density = sum(1 for c in last_segment_clean[-15:] if c.isdigit()) / max(1, min(15, len(last_segment_clean)))
                
                is_product = False
                
                # Logica prodotto
                if ends_with_number:  # Finisce con numero = molto probabilmente SKU
                    is_product = True
                elif words_in_segment >= 4:  # Path lungo descrittivo = prodotto
                    is_product = True
                elif digit_density > 0.15:  # >15% numeri = ha ID/SKU
                    is_product = True
                elif has_sku_pattern and words_in_segment >= 2:  # Ha numeri E almeno 2 parole
                    is_product = True
                
                if is_product:
                    categories['Prodotti'].append(row)
                else:
                    # CATEGORIA: path corto, poche parole, senza numeri
                    # Tipicamente: /abiti/, /rossetti/, /auto-storiche/
                    categories['Categorie'].append(row)
            else:
                # Path vuoto o solo /
                categories['Categorie'].append(row)
